...
QueueArray.prototype.enqueue = function (elemToEnqueue) {
  ...
  if (explanation) {
    explanation.innerText = `Enqueued value "${elemToEnqueue}" to the queue. The tail now points to index ${this.tail}.`;
  }
...
QueueArray.prototype.dequeue = function (ignored) {
  ...
  const explanation = document.getElementById("explanationText");
  if (explanation) {
    explanation.innerText = `Dequeued value "${dequeuedVal}" from the queue. The head now points to index ${this.head}.`;
  }
...
QueueArray.prototype.clearAll = function () {
  ...
  const explanation = document.getElementById("explanationText");
  if (explanation) {
    explanation.innerText = "The queue is cleared. Both head and tail reset to index 0.";
  }
