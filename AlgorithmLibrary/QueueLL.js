...
QueueLL.prototype.resetLinkedListPositions = function () {
  for (var i = this.top - 1; i >= 0; i--) {
    var nextX =
      ((this.top - 1 - i) % LINKED_LIST_ELEMS_PER_LINE) * LINKED_LIST_ELEM_SPACING + LINKED_LIST_START_X;
    var nextY =
      Math.floor((this.top - 1 - i) / LINKED_LIST_ELEMS_PER_LINE) * LINKED_LIST_LINE_SPACING + LINKED_LIST_START_Y;
...
QueueLL.prototype.enqueue = function (elemToPush) {
  ...
  // Dynamic explanation box update for Enqueue
  const explanation = document.getElementById("explanationText");
  if (explanation) {
    explanation.innerText = `Enqueued value "${elemToPush}" to the queue. The tail now points to the new element.`;
  }
...
QueueLL.prototype.dequeue = function (ignored) {
  ...
  // Dynamic explanation box update for Dequeue
  const explanation = document.getElementById("explanationText");
  if (explanation) {
    explanation.innerText = `Dequeued value "${this.arrayData[this.top]}" from the queue. The head now points to the next element.`;
  }
...
QueueLL.prototype.clearAll = function () {
  ...
  // Dynamic explanation box update for Clear Queue
 const explanation = document.getElementById("explanationText");
  if (explanation) {
    explanation.innerText = "The queue is cleared. Both head and tail reset.";
  }
