...
StackArray.prototype.enableUI = function () {
  for (var i = 0; i < this.controls.length; i++) {
    this.controls[i].disabled = false;
  }
};

StackArray.prototype.disableUI = function () {
  for (var i = 0; i < this.controls.length; i++) {
...
StackArray.prototype.pushCallback = function () {
...
StackArray.prototype.popCallback = function () {
  if (this.top > 0) {
    this.implementAction(this.pop.bind(this), "");
  }
};

StackArray.prototype.clearCallback = function () {
  this.implementAction(this.clearData.bind(this), "");
};

StackArray.prototype.clearData = function () {
  this.commands = [];
  this.clearAll();
...
StackArray.prototype.push = function (elemToPush) {
  ...
  if (explanation) {
    explanation.innerText = `The value "${elemToPush}" is pushed onto the stack. The stack grows by one, and the new top now points to index ${this.top}.`;
  }
...
StackArray.prototype.pop = function () {
  ...
  this.top--;
  let poppedValue = this.arrayData[this.top];
  ...
  this.cmd("CreateLabel", labPopValID, poppedValue, xpos, ypos);
  ...
  this.cmd("Delete", this.highlight1ID);
  this.cmd("SetText", this.leftoverLabelID, "Popped Value: " + poppedValue);

  const explanation = document.getElementById("explanationText");
  if (explanation) {
    explanation.innerText = `The value "${poppedValue}" is popped from the stack. The stack shrinks by one, and the top now points to index ${this.top}.`;
  }
...
StackArray.prototype.clearAll = function () {
  ...
  const explanation = document.getElementById("explanationText");
if (explanation) {
  explanation.innerText = `The stack has been cleared. All values are removed, and the top is reset to 0.`;
}
  return this.commands;
};
