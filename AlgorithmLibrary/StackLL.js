// Modified StackLL.js with Explanation Box Updates
...
StackLL.prototype.push = function(elemToPush) {
  ...
  "Pushed value \"" + elemToPush + "\" to the stack. A new node is created and added at the beginning of the linked list. This new node becomes the new top of the stack.";
...
StackLL.prototype.pop = function(ignored) {
  ...
  if (this.top === 0) return this.commands; // Nothing to pop

  const poppedValue = this.arrayData[this.top - 1];
  ...
  this.cmd("CreateLabel", labPopValID, poppedValue, LINKED_LIST_START_X, LINKED_LIST_START_Y);
  ...
  // ✅ SAFELY update explanation text (only if element exists)
  const explanation = document.getElementById("explanationBox");
  if (explanation) {
    explanation.innerText = `The value "${poppedValue}" was removed from the top of the stack. The previous node is now the new top.`;
  }
...
StackLL.prototype.clearAll = function() {
  ...
  this.cmd("SetNull", this.topID, 1);
    const explanation = document.getElementById("explanationBox");
  if (explanation) {
    explanation.innerText = "The stack has been cleared. All nodes have been removed and the top is reset to null.";
  }
  return this.commands;
};
